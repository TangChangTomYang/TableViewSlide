...
static WLSideslipCellProxy *__proxy = nil;
//尝试绑定proxy进行滚动的代理拦截
- (void)tryBindProxy {
    ...
    if ([tableView isKindOfClass:[UITableView class]]) {
        ...
        if (![NSStringFromClass([tableView.delegate class]) isEqualToString:@"WLSideslipCellProxy"]) {
//        if (![tableView.delegate isKindOfClass:[WLSideslipCellProxy class]]) {
            
            //保证一个tableView只会设置一次proxy
            WLSideslipCellProxy *proxy = [WLSideslipCellProxy alloc];
            __proxy = proxy;
